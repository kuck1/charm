...
// I) Get some scratch on the host
   ...
   double cpu1,cpu2,cpu3,cpu4, mult1_start, mult1_end, mult2_start, mult2_end, mult3_start, mult3_end, total1=0, total2=0, total3=0, cpu5, cpu6;
   cputime(&cpu1);
...
// IV) Schulz iteration
  ...
  // A) Initialize scr_mat1 and Titer on host
    ...
    // scr_mat1 = S/2
    for(int i=0;i<nstate_sq;i++){scr_mat1[i] = S[i]/2.0;}    // DO THIS ON GPU WITH KERNEL ****
    // Titer = I = unit matrix
    memset(Titer,0,nstate_sq*sizeof(double));                   // DO THIS ON GPU WITH KERNEL ****
    for(int i=0;i<nstate;i++){int ind = i+i*nstate;Titer[ind] = 1.0;} // DO THIS ON GPU WITH KERNEL ****
    ...
    while (tol_now > 1.0e-15 && iter<10){
        ...
        int itransp  = 0;    int inorm    = 1;                  //Take declarations outside of the while loop, create alpha0, alpha1, alpha2
        double alpha = -1.0; double beta  = 1.0;
        memset(scr_mat2,0,nstate_sq*sizeof(double));                       // DO THIS ON GPU WITH KERNEL ****
        for(int i=0;i<nstate;i++){int ind = i+i*nstate;scr_mat2[ind]=3.0;} // DO THIS ON GPU WITH KERNEL **** Keep on kernel for reuse
        cublasSetMatrix(nstate, nstate, sizeof(double), scr_mat2, nstate, d_mat2, nstate); //eliminate this step
        // initialize on device d_mat2 to be eqaul to 3I, 4 (or more in case extra iterations are needed of these can be ready in advance)
	cputime(&mult1_start);
	...
	cputime(&mult1_end);
        //--------------------------------
        // scr_mat1 = 0.5*scr_mat1*scr_mat2 = 0.5*scr_mat3*scr_mat2                 // Run this step concurently with the next step ?
        alpha = 0.5;  beta  = 0.0;
        cudaMemcpy(d_mat3,d_mat1,nstate_sq*sizeof(double),cudaMemcpyDeviceToDevice); //eliminate this step
        cputime(&mult2_start);
	...
	total1 += mult1_end - mult1_start;
	total2 += mult2_end - mult2_start;
	total3 += mult3_end - mult3_start;	
...
// VII) Clean up host
    ...
    cputime(&cpu2);
    printf("time in while loop %g : time copying result at end %g : initailization time %g : mallocing time %g\n", cpu6-cpu5, cpu4-cpu6, cpu5-cpu3, cpu3-cpu1);
    printf("mult1 %g : mult2 %g :mult3 %g\n", total1, total2, total3);
