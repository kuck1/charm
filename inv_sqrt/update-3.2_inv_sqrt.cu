...
// II) d_mat3 = (d_mat3 - d_Titer) ^ 2
    ...
    if(global_idx < nstate_sq){                    // Check if this will ever be needed
        double tmp = d_mat3[global_idx] - d_Titer[global_idx];
        d_mat3[global_idx] = tmp * tmp;
    }
    __syncthreads();

// IV) Reduce any sized array

    unsigned int length = 1024, split_idx = 0;

    if((blockIdx.x + 1) == gridDim.x) {   // This is the last block and may not be full
        length = nstate_sq - 1024 * (gridDim.x - 1);
    }

    for(length ; length != 1; length = split_idx){   // Ultimately we want host to do ~ 2 ^ 13 work, here it does 2^10
        split_idx = (blockDim.x * blockIdx.x) + (length + 1) / 2;
        unsigned int block2_size = length / 2;    //length - split_idx;
        //if(local_idx == 4){ PRINTF("length: %d, split_idx: %d, block2_size: %d \n", length, split_idx, block2_size);}
        if(split_idx <= global_idx && global_idx < length){ 
            d_mat3[global_idx - block2_size] += d_mat3[global_idx];
        }
        __syncthreads();
    }

// V) Only 0 thread thread from each block copies result back write
...
// IV) Schulz iteration
  ...
  // A) Initialize scr_mat1 and Titer on host
	...
	tol_now = 0;
        for(int i = 0; i < (nstate_sq + 1023)/1024; i ++){
