...
// I) Get some scratch on the host
   ...
   double *scr_mat1  = new double[nstate_sq];
   double *scr_mat2  = new double[nstate_sq];
   double *scr_mat3  = new double[nstate_sq]; 			// DO NOT NEED THESE ANYMORE, REMOVE TO TEST TIMING
...
// II) Set up CUBLAS context
    ...
    // cudaError_t cudaStat;           // cudaMalloc status 
    // cublasStatus_t stat;            // CUBLAS functions status 
    cublasHandle_t handle0, handle1;              // CUBLAS context
//============================================================================
// III) Allocate memory on the device
    ...
    cublasCreate(&handle0);                           // initialize CUBLAS context
...
// A) Initailize d_mat1 and d_Titer on device
	...
	cputime(&mult1_start);
        device_matmul(d_Titer,&nstate,&inorm,d_mat1,&nstate,&itransp,d_mat2,
            &nstate,&nstate,&nstate,&nstate,&alpha0,&beta0,handle0);
	...
	device_matmul(d_mat3,&nstate,&inorm,d_mat2,&nstate,&itransp,d_mat1,
            &nstate,&nstate,&nstate,&nstate,&alpha1,&beta1,handle0);
        /////cudaThreadSynchronize();
        cputime(&mult2_end);
	//--------------------------------
	...
	cputime(&mult3_start);
        device_matmul(d_mat2,&nstate,&inorm,d_mat3,&nstate,&itransp,d_Titer,
            &nstate,&nstate,&nstate,&nstate,&alpha1,&beta1,handle1);
        cublasSetStream(handle0, streams[0]);
	cudaThreadSynchronize();
        cputime(&mult3_end); 
