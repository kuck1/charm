...
void cputime(double *);
double get_iter_Tmat(double *,double *,int );
...
// II) Try three methods
   ...
   for(int i = 0; i < 1000; i++){
	sum += get_iter_Tmat(S,Titer,nstate);
   }

   printf("The average time taken is %g ms for Update-2.0\n\n", sum);
...
double get_iter_Tmat(double *S,double *Titer,int nstate)
...
// IV) Schulz iteration
  ...
  // A) Initialize scr_mat1 and Titer on host
    ...
    int iter        = 0;
    //double tol_now  = 1.0;
    while (iter < 4){ //(tol_now > 1.0e-15 && iter<10){
        ...
        //PRINTF("iter %d : tol %g\n",iter,tol_now);
   //     PRINTF("iter %d : Skipping tolerence check, guessing 4 iterations should be done\n", iter);
    }//endwhile
