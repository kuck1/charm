...
// A) Initailize d_mat1 and d_Titer on device
    ...
    // d_mat1 = S/2   **** -> d_mat1 = S
    cublasSetMatrix(nstate, nstate, sizeof(double), S, nstate, d_mat1, nstate);
    ...
    int itransp  = 0;    int inorm    = 1;  
    double alphaSpecial0 = -0.5, alphaSpecial1 = 0.25;
	...
	if(iter == 1){
            //STEP-1--------------------------------
            // d_mat2 =  3*I - 0.5*d_Titer*d_mat1 
	    device_matmul(d_Titer,&nstate,&inorm,d_mat1,&nstate,&itransp,d_mat2,
	        &nstate,&nstate,&nstate,&nstate,&alphaSpecial0,&beta0,handle);
            //STEP-2--------------------------------
            // d_mat1 = 0.25*d_mat1*d_mat2 = 0.25*d_mat3*d_mat2                 // Run this step concurently with the next step ?
	    ...
	    device_matmul(d_mat3,&nstate,&inorm,d_mat2,&nstate,&itransp,d_mat1,
                &nstate,&nstate,&nstate,&nstate,&alphaSpecial1,&beta1,handle);
	} 
	else{
 	    //STEP-1--------------------------------
