...
/*******
The host code expects the kernel below to populate the d_tol_array:

tolerance_check<<<(nstate_sq + 1023)/1024, 1024>>>(d_tol_now, d_Titer, d_mat3, nstate_sq);
cudaMemcpy(tol_now_ptr, d_tol_now, ((nstate_sq + 1023)/1024)*sizeof(double), cudaMemcpyDeviceToHost);
*******/
...
{//begin routine
    ...
    if(threadIdx.x == 0){
        PRINTF("global_idx: %d, local_idx: %d\n", threadIdx.x + blockDim.x * blockIdx.x, threadIdx.x);
        PRINTF("blockDim.x: %d, blockIdx.x: %d, gridDim.x: %d\n\n", blockDim.x, blockIdx.x, gridDim.x);
    }
...
// III) Reduce any sized array
    if(global_idx < nstate_sq){                                 // Needed in case last block is not full
        ...
        for( ; length != 1; length = next_length){   // Ultimately we want host to do ~ 2 ^ 13 work, here it does 2^10
            ...
            if(local_idx == 0){ PRINTF("length: %d, global_split_idx: %d, block2_size: %d \n", length, global_split_idx, block2_size);}
...
// I) Get some scratch on the host
   ...
   int nstate_sq     = nstate*nstate;
   double *tol_now_ptr = new double[1024];
   double *scr_mat1  = new double[nstate_sq];
   double *scr_mat2  = new double[nstate_sq];
...
// III) Allocate memory on the device
    ...
    cudaMalloc(&d_tol_now, 1024*sizeof(double)); 
    puts("memory allocated on device");
    cublasCreate(&handle);                           // initialize CUBLAS context
...
// IV) Schulz iteration
  ...
  // A) Initialize scr_mat1 and Titer on host
    ...
    while (tol_now > 1.0e-15 && iter<10){
        ...
        cublasGetMatrix(nstate, nstate, sizeof(double), d_mat3, nstate, scr_mat3, nstate);    // Needed for old method
        cublasGetMatrix(nstate, nstate, sizeof(double), d_Titer, nstate, Titer, nstate);      // Needed for old method
        
        int grid_size = (nstate_sq + THREADS_PER_BLOCK - 1)/THREADS_PER_BLOCK;
        printf( "Entering kernel code, grid_size: %d\n", grid_size);
        tolerance_check<<<grid_size, THREADS_PER_BLOCK>>>(d_tol_now, d_Titer, d_mat3, nstate_sq); // grid_size, 1024
        
        gpuErrchk( cudaPeekAtLastError() );
        puts("No launch error, error in kernel");
        
        gpuErrchk( cudaDeviceSynchronize() );
        puts("Nevermind, no error in kernel");

        cudaMemcpy(tol_now_ptr, d_tol_now, grid_size*sizeof(double), cudaMemcpyDeviceToHost);
        puts("copied array from device to host");
        tol_now = 0.0;
        ...
        puts("checking tolerance using old method");
        // Check tolerance using old method
        double actual_tol = 0.0;
        for(int i=0;i<nstate_sq;i++){
            double tmp=scr_mat3[i]-Titer[i];
            actual_tol += tmp*tmp;
        }//endfor
        actual_tol /= ((double)nstate_sq);
        actual_tol = sqrt(actual_tol);
        PRINTF("iter %d : actual_tol %g : new_tol %g\n\n",iter, actual_tol, tol_now);
        tol_now = actual_tol;     // THIS JUST LIMITS THE NUMBER OF ITERATIONS TO WHAT IT SHOULD BE
    }//endwhile
