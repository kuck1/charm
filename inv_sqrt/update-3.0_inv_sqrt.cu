...
// III) Reduce
    ...
    for(i = (blockDim.x / 2); i > 0; i >>= 1){
        if(tid < i){
            d_mat3[idx] += d_mat3[idx + i];
        }
        __syncthreads();
    }

// IV) Only 0 thread writes result

    if(tid == 0){
        *d_tol_now = sqrt(d_mat3[i] / ((double)nstate_sq));
    }
}
/*==========================================================================*/
...
// I) Get some scratch
   ...
   int nstate_sq     = nstate*nstate;
   double *tol_now_ptr = new double[1];
   double *scr_mat1  = new double[nstate_sq];
   double *scr_mat2  = new double[nstate_sq];
...
// III) Allocate memory on the device
    ...
    cudaMalloc(&d_tol_now, sizeof(double)); 
...
// IV) Schulz iteration
  ...
  // A) Initialize scr_mat1 and Titer on host
    ...
    while (tol_now > 1.0e-15 && iter<10){
        ...
        cudaMemcpy(tol_now_ptr, d_tol_now, sizeof(double), cudaMemcpyDeviceToHost);
        tol_now = *tol_now_ptr;
