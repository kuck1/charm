...
// II) d_mat3 = (d_mat3 - d_Titer) ^ 2
    ...
    if(idx < nstate_sq){                    // Check if this will ever be needed
        double tmp = d_mat3[idx] - d_Titer[idx];
        d_mat3[idx] = tmp * tmp;
    }
    __syncthreads();
...
// IV) Reduce any sized array
    ...
    unsigned int length, split_idx = 0;
    for(length = nstate_sq; length != 2; length = split_idx){   // 2 will become 2 ^ 13
        split_idx = ((length + 1) / 2);
        //if(tid == 0){ PRINTF("length: %d, split_idx: %d", length, split_idx);}
	unsigned int block2_size = length - split_idx;
        if(split_idx <= idx && idx < length){ 
            d_mat3[idx - block2_size] += d_mat3[idx];
        }
        __syncthreads();
    }

// V) Only 0, 1 threads write

    if(tid < 3){
        d_tol_now[tid] = d_mat3[tid];
    }
}
/*==========================================================================*/
...
// I) Get some scratch
   ...
   int nstate_sq     = nstate*nstate;
   double *tol_now_ptr = new double[2];
   double *scr_mat1  = new double[nstate_sq];
   double *scr_mat2  = new double[nstate_sq];
...
// III) Allocate memory on the device
    ...
    cudaMalloc(&d_tol_now, 2*sizeof(double)); 
...
// IV) Schulz iteration
  ...
  // A) Initialize scr_mat1 and Titer on host
    ...
    while (tol_now > 1.0e-15 && iter<10){
        ...
        tolerance_check<<<1, nstate_sq>>>(d_tol_now, d_Titer, d_mat3, nstate_sq);
        cudaMemcpy(tol_now_ptr, d_tol_now, 2*sizeof(double), cudaMemcpyDeviceToHost);
        
        tol_now = sqrt((tol_now_ptr[0] + tol_now_ptr[1]) / nstate_sq);       // This will need to be converted into for loop for bigger values
