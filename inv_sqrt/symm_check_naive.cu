...
int symm_check(double * array, int n);
...
// II) Schulz iteration
  ...
  // B) Iterate
   ...
   while (tol_now > 1.0e-15 && iter<10){
     ...
     printf("iter %d : Titer %d : scr_mat1 %d\n", iter, symm_check(Titer, nstate), symm_check(scr_mat1, nstate));
     matmul(Titer,&nstate,&inorm,scr_mat1,&nstate,&itransp,scr_mat2,
               &nstate,&nstate,&nstate,&nstate,&alpha,&beta);
     printf("iter %d : scr_mat2 %d\n", iter, symm_check(scr_mat2, nstate));
     //--------------------------------
     // scr_mat1 = 0.5*scr_mat1*scr_mat2 = 0.5*scr_mat3*scr_mat2
     alpha = 0.5;  beta  = 0.0;
     memcpy(scr_mat3,scr_mat1,nstate_sq*sizeof(double));
     matmul(scr_mat3,&nstate,&inorm,scr_mat2,&nstate,&itransp,scr_mat1,
               &nstate,&nstate,&nstate,&nstate,&alpha,&beta);
     printf("iter %d : scr_mat3 %d\n", iter, symm_check(scr_mat3, nstate));
     //--------------------------------
     // Titer = 0.5*scr_mat2*Titer = 0.5*scr_mat2*scr_mat3
     alpha = 0.5;  beta  = 0.0;
     memcpy(scr_mat3,Titer,nstate_sq*sizeof(double));
     matmul(scr_mat2,&nstate,&inorm,scr_mat3,&nstate,&itransp,Titer,
               &nstate,&nstate,&nstate,&nstate,&alpha,&beta);
     //--------------------------------
     // tolerence check
     tol_now = 0.0;
...
int symm_check(double * array, int n){
    int i, j, coord, symm_coord, result = 0;

    for(i = 0; i < n; i ++){
        for(j = 0; j < n; j++){
            coord = i + n * j;
            symm_coord = j + n * i;
            if (array[coord] != array[symm_coord]){
                result++;
            }
        }
    }
